...
#include <iostream>		//Das  Lädt die Bibliothek glaub ich... (update: JA tut es)
	//+++ um so mehr + vor dem kommentar steht, um so wichtiger ist er.( ich werde wahrscheinlich schon ab zeile 30 damit aufhören weil es soviel kommentare sind(update: ich kann gut schätzen)
...
{		//+++mit der komischen klammer fängt das programmieren erst richtig an, ausser du kappierst hier nix.
	...
	std::cout << "TEST 256 (ich hab eh irgendwann mal vergessen zu zählen, es fühlte sich aber ganz genau so an wie es da steht.)";//+++alles was im "HALLO! hier" ist wird als text ausgegeben.
